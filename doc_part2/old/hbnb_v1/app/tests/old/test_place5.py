...
class TestPlace(unittest.TestCase):
    ...
    def test_title_max_length(self):
        """Test that a title with exactly 100 characters does not raise ValidationError."""
        max_length_title = "x" * 100  # Create a title with exactly 100 characters
        try:
            place = Place(title=max_length_title, description="A description.", price=100.0,
                          latitude=34.0522, longitude=-118.2437, owner=self.user)
            self.assertEqual(place.title, max_length_title)
        except ValidationError:
            self.fail("Place creation raised ValidationError unexpectedly with 100 characters in title")
