...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Protect access to add_review.html
  const currentPage = window.location.pathname;
  if (currentPage.includes("add_review.html") && (!isLoggedIn || userEmail !== adminEmail)) {
    alert("Only admin can access this page.");
    window.location.href = "login.html";
  }
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", (e) => {
      ...
      if (email === adminEmail && password === "admin1234") {
        localStorage.setItem("isLoggedIn", "true");
        localStorage.setItem("userEmail", email);
        alert("Login successful!");
        const redirectUrl = new URLSearchParams(window.location.search).get("redirect");
        ...
        alert("Invalid credentials. Only admin is allowed.");
      }
    });
  }
  ...
  if (addReviewButton) {
    addReviewButton.addEventListener("click", () => {
      if (isLoggedIn && userEmail === adminEmail) {
        ...
        alert("Only admin can add reviews.");
        window.location.href = "login.html";
      }
    });
  }
