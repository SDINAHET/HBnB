...
class APITestCase(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        with open('test_log_amenity.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Log'])
            writer.writerow([log_contents])
        print("Test log successfully written to test_log.csv")
    ...
    def test_unauthorized_place_update(self):
        ...
        logging.info(f"Test Unauthorized Place Update: {response.status_code} {response.json()}")
    ...
    def test_get_all_amenities(self):
        ...
        logging.info(f"Test Get All Amenities: {response.status_code}")
        logging.info(response.json())
        self.assertEqual(response.status_code, 200)

    def test_get_amenity_by_id(self):
        url = f"{BASE_URL}/amenities/{self.amenity_id_1}"
        headers = {"Authorization": f"Bearer {self.token}"}
        response = requests.get(url, headers=headers)
        logging.info(f"Test Get Amenity by ID: {response.status_code}")
        logging.info(response.json())
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_amenity(self):
        ...
        response = requests.put(url, json=data, headers=headers)
        logging.info(f"Test Update Amenity: {response.status_code}")
    ...
    def test_create_amenity(self):
        ...
        response = requests.post(url, json=data, headers=headers)
        logging.info(f"Test Create Amenity: {response.status_code}")
        logging.info(response.json())
        self.assertEqual(response.status_code, 201)

print(""" TEST AMENITY PART3
██╗  ██╗██████╗ ███╗   ██╗██████╗
██║  ██║██╔══██╗████╗  ██║██╔══██╗
███████║███████║██╔██╗ ██║███████║
██╔══██║██║  ██║██║╚██╗██║██║  ██║
