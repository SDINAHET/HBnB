...
class ReviewAPITestCase(unittest.TestCase):
    ...
    @staticmethod
    def create_review(token, text, rating, place_id):
    ...
    def test_create_review_as_regular_user(self):
        # User 2 creates a review for User 1's place
    ...
    @unittest.skipIf(setup_failed, "Skipping test due to failed setup.")
    def test_create_review_as_owner_should_fail(self):
        # User 1 tries to create a review for their own place
        url = f"{BASE_URL}/reviews/"
        ...
        response = requests.post(url, json=data, headers=headers)
        print(f"Test Create Review as Owner: {response.status_code}, Response: {response.json()}")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json().get('message'), "You cannot review your own place.")
    ...
    def test_create_duplicate_review_should_fail(self):
        ...
        url = f"{BASE_URL}/reviews/create_review"
    ...
    def test_delete_review_as_owner(self):
        ...
        # User 2 deletes their review
        url = f"{BASE_URL}/reviews/{review_id}"
        headers = {"Authorization": f"Bearer {self.user2_token}"}
        response = requests.delete(url, headers=headers)
        print(f"Test Delete Review as Owner: {response.status_code}")
        self.assertEqual(response.status_code, 200)
